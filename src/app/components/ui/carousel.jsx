  ...
  ({ className, variant, size = "icon", ...props }, ref) => {
    ...
    return (
      <Button
        ...
        size={size}
        className={cn(
          "absolute scale-75 sm:scale-100  h-8 w-8 rounded-full group",
          orientation === "horizontal"
            ? "-left-8 sm:-left-12 -translate-y-1/2"
            : "-top-12 left-1/2 -translate-x-1/2 rotate-90",
          variant == "teams" ? "top-1/3" : "top-1/2",
          className
        )}
      ...
      >
        <ArrowLeftIcon
          className={`h-4 w-4 ${
            variant == "teams" ? "text-[#C89E6D] group-hover:text-black" : ""
          }`}
        />
...
const Tabnext = React.forwardRef(({ item, handlenav, setActiveTab, activeTab, index }, ref) => {
  const { scrollTo, scrollNext } = useCarousel();
  return (
    <button
      onClick={(e) => {
        handlenav(item);
      ...
      className={`shrink-0 font-lato font-bold md:text-xl rounded-lg p-1  hover:bg-[#C89E6D] ${
        activeTab == item ? "bg-[#C89E6D] text-gray-800" : "text-white"
